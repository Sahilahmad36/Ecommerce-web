...
const Add = ({token}) => {
    ...
    const [subCategory, setSubCategory] = useState("outfit");
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
        ...
        <div className='w-full'>
            <p className='mb-2'>Product(optional)</p>
            <input onChange={(e)=>setAbout(e.target.value)} value={about} className='w-full max-w-[500px] px-3 py-2' type="text" placeholder='Type about product here' />
        </div>
        <div className='w-full'>
        ...
        <div>
            ...
            <div className="flex gap-3">
                <div onClick={() => handleSizeToggle("XS")}>
                ...
                <div onClick={() => handleSizeToggle("Red")}>
                <p className={`${sizes.includes("Red") ? 'bg-pink-200' : 'bg-slate-200'} px-3 py-1 cursor-pointer`}>Red</p>
                </div>
                <div onClick={() => handleSizeToggle("Blue")}>
                <p className={`${sizes.includes("Blue") ? 'bg-pink-200' : 'bg-slate-200'} px-3 py-1 cursor-pointer`}>Blue</p>
                </div>
                <div onClick={() => handleSizeToggle("White")}>
                <p className={`${sizes.includes("White") ? 'bg-pink-200' : 'bg-slate-200'} px-3 py-1 cursor-pointer`}>White</p>
                </div>
                <div onClick={() => handleSizeToggle("Black")}>
                <p className={`${sizes.includes("Black") ? 'bg-pink-200' : 'bg-slate-200'} px-3 py-1 cursor-pointer`}>Black</p>
                </div>
                <div onClick={() => handleSizeToggle("Yellow")}>
                <p className={`${sizes.includes("Yellow") ? 'bg-pink-200' : 'bg-slate-200'} px-3 py-1 cursor-pointer`}>Yellow</p>
                </div>
                <div onClick={() => handleSizeToggle("Green")}>
                <p className={`${sizes.includes("Green") ? 'bg-pink-200' : 'bg-slate-200'} px-3 py-1 cursor-pointer`}>Green</p>
                </div>
            </div>
        </div>
